...
const coverSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <!-- Logo icon: original vb=0 0 82 51, translate(-9.52,-47.10), scale=${logoScale.toFixed(4)} -->
  <g transform="translate(${groupX}, ${groupY}) scale(${logoScale.toFixed(4)}) translate(-9.5249995,-47.095833)">
    <path fill="url(#logoGrad)" d="${logoPath}"/>
  </g>
  ...
  <text
    ...
    dominant-baseline="auto"
    letter-spacing="0.2"
  >Sve što trebaš za coaching.</text>
</svg>`

const outPath = path.join(__dirname, '..', 'public', 'unitlift-cover-820x312.png')
